  ...
  try {
    ...
    const mailOptions = {
      ...
      html: `<h1>Recuperar contraseña</h1>
        <hr>
        <p>Tu token de seguridad es el siguiente <b>${tokenUuid}</b></p>
        <p>¡No lo compartas con nadie!</p>
      `,
    };

    // send email
    const resultSendEmail = await sendEmail(email, mailOptions);
...
const updatePassword = async (req, res = response) => {
  try {
    ...
    if (!changePasswordToken || !email || !newPassword)
